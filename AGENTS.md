...
**statix** lints the tree per `statix.toml` at the repo root (which the
emacs flymake nix-mode backend reads too, so editor and check agree), and
**deadnix** (`--no-lambda-pattern-names`, mandatory here — see the comment on
`checks.deadnix`) finds bindings nothing references. A repo-local pre-commit
hook runs both before every commit that touches `.nix` files. The hook is generated by the `default` devShell and installed by its
`shellHook`, so entering the repo with direnv (`.envrc`: `use_flake`) — or a
bare `nix develop` — is what puts it in `.git/hooks/pre-commit`, as a symlink
to the store. Nothing to remember per clone, and the statix it runs is this
flake's, not the ambient one.

It replaces the machine-wide hook seeded by `init.templatedir` and chains
back to it at runtime, so the gitleaks scan still runs. Remove it by
restoring that copy: `cp "$(git config --get init.templatedir)/hooks/pre-commit" .git/hooks/`.
