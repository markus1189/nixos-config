...
emacs flymake nix-mode backend reads too, so editor and check agree). An
repo-local pre-commit hook runs it before every commit that touches `.nix`
files. The hook is generated by the `default` devShell and installed by its
